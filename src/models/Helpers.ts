...
export const Helpers = {
  ...
  getMeshesByName(name: string, partly = false) {
    return globalThis.scene.meshes.filter((mesh) =>
      Helpers.IsName(mesh.name, name, partly)
    ) as Array<Mesh>;
  },
  ...
  drawEllipsoid(mesh: AbstractMesh) {
    const colliderMesh = MeshBuilder.CreateSphere(
      'colliderMesh',
      {
        segments: 16,
        diameterX: mesh.ellipsoid.x * 2,
        diameterY: mesh.ellipsoid.y * 2,
        diameterZ: mesh.ellipsoid.z * 2,
      },
      scene
    );
