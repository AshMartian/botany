...
export default class Environment {
  ...
  setupFog() {
    this.scene.fogColor = new Color3(
      this.settings.fog.color.r,
      this.settings.fog.color.g,
      this.settings.fog.color.b
    );
  ...
  setupLightAndShadow() {
    ...
    if (!light) {
      // Create new directional light with proper settings
      light = new DirectionalLight('MainDirectionLight', new Vector3(-1, -2, -1), this.scene);
      light.position = new Vector3(20, 40, 20);
      light.intensity = 1.0; // Default value
  ...
  setupSkybox() {
    ...
    skyboxMaterial.reflectionTexture = new CubeTexture(
      './resources/graphics/textures/skybox/skybox',
      scene
    );
