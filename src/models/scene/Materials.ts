...
export default class Materials {
  static addCustomMaterial() {
    const alphaTags = ['material_glass'];
  ...
  static async addMaterial(meshes: Array<AbstractMesh>): Promise<null> {
    ...
    for (const mesh of meshes) {
      ...
      let nodeMaterial = materials.find(
        (materialItem) => materialItem.name === materialName
      ) as null | NodeMaterial;

      if (!nodeMaterial) {
        nodeMaterial = await NodeMaterial.ParseFromFileAsync(
          materialName,
          './resources/graphics/materials/' + materialName + '.json',
          scene
        );
