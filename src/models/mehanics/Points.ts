...
export default class Points {
  ...
  init() {
    this.points.forEach((point: Mesh) => {
      ...
      point.metadata = {
        attracted: false,
        run: false,
        originCoordinate: { x: point.position.x, y: point.position.y, z: point.position.z },
      };
      ...
      this.points.forEach((point: Mesh) => {
        if (point.metadata?.attracted && !point.metadata.run) {
          ...
          const speed = Math.max(
            0.2,
            point.position.subtract(this.meshFoot.position).length() / 20
          );
  ...
  createParticleSystem() {
    ...
    this.particleSystem.particleTexture = new Texture(
      './resources/graphics/textures/flower.png',
      globalThis.scene
    );
