...
export default class Collisions {
  ...
  private setCollisions() {
    this.listCollisions = this.scene.meshes.filter(
      (mesh) => mesh.checkCollisions || Helpers.hasTag(mesh, 'collision')
    );
  ...
  private static checkCollisionList(mesh: AbstractMesh, list: Array<AbstractMesh>) {
    for (const keyMesh in list) {
      ...
      if (
        listMesh !== mesh &&
        mesh.intersectsMesh(listMesh, Tags.MatchesQuery(listMesh, 'precise'))
      ) {
        return listMesh;
      }
    }
